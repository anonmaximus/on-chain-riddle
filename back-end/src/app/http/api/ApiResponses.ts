...
import HttpCodes from "#App/http/HttpCodes";
...
export default abstract class ApiResponses {
	...
	public static httpGone(response: Response, responseData: IResponseData = "Gone") {
		return this.httpResponse(response, HttpCodes.GONE, responseData);
	}
	...
	public static httpResponse(response: Response, httpCode: HttpCodes, responseData: IResponseData = {}) {
		...
		response.status(httpCode).send(responseData);
	}
}
